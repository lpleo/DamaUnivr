...
public class IA {
	...
	public IA(CampoDiGioco campo){
		...
		for (int c=0; c<8; c++)
			for (int i=0; i<8; i++)
					if((c+i)%2 == 0){
						//System.out.println("Controllo finale n° 1\nX:"+c+" Y:"+i);
						pesata [c][i]+= EatCoder(c,i,9,9,9,9,VirtualCampo);
						//System.out.println("Controllo finale n° 2\nX:"+c+" Y:"+i+" Peso:"+peso[c][i]);
						if (pesata [c][i] > max){	
							max = pesata[c][i];
							Xmax = c;
							Ymax = i;
		...
		if ( max == 0 ){
			//System.out.println("Controllo finale n° 3\nNon posso mangiare quindi controllo se muovo");
			for (int c=0; c<8; c++)
				for (int i=0; i<8; i++)
					if((c+i)%2==0){
						//System.out.println("Controllo finale n° 4\nX:"+c+" Y:"+i);
						peso[c][i] = MoveCoder(c,i,false);	
						//System.out.println("Controllo finale n° 5\nX:"+c+" Y:"+i+" Peso:"+peso[c][i]);
						if (peso [c][i] > max){	
							max = peso[c][i];
							Xmax = c;
							Ymax = i;
						}else
							////System.out.println("Controllo finale n° 6\nPer caso è il random?\nX:"+c+" Y:"+i+" Peso:"+peso[c][i]);
			...
			if (max == 0){
				//System.out.println("Controllo finale n° 7\nnon posso muovere");
			}else{
				////System.out.println("Controllo finale n° 8\nPosso muovere X:"+Xmax+" Y:"+Ymax);
				muovi(Xmax,Ymax);
				//System.out.println("e qui?");
				
			}
		}else{
			//System.out.println("Controllo finale n° 9\nPosso mangiare X:"+Xmax+" Y:"+Ymax);
	...
	private  CampoDiGioco settaSp(int x, int y,CampoDiGioco campo) {
		campo.SetXp(x);
		campo.SetYp(y);
		campo.SetSp(campo.toStringState(x, y));
		return campo;
	}
	...
	private int EatCoder(int x, int y,int s, int xp, int yp, int sp,CampoDiGioco campo) {
